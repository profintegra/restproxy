...
public class MetadataForm extends Controller {
	...
	public View get(HttpServletRequest request, PathParser pathInfo) throws Exception {
		...
		if(StringUtils.isNotBlank(path)){			
			String body = Request.excuteGet(UrlConstants.HOST + path, new HeaderBuilder().acceptAll().authorization(request.getHeader(UrlConstants.AUTH_HEADER)).build());			
			JSONObject json = XML.toJSONObject(body);
			String prettyJson = json.toString(1);
			return new JsonView(prettyJson);			
		}		
